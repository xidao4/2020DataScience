...
def get_allAnswers(path):       #获取指定路径下的参考答案  变为数组
    try:
        ...
        print("找不到对应的参考答案testCases.json")
        return None
...
#f=open("sample.json",encoding="utf8")    #样本数据
f=open("test_data.json",encoding="utf8")    #全部数据
res=f.read()
data=json.loads(res)
...
for user in data:
    # global record_idx
    cases=user["cases"]
    #submit_pre = "s_submit_code\\user_" + str(user["user_id"])   #样本
    submit_pre = "submit_code\\user_" + str(user["user_id"])    #全部
    #pro_pre = "answer_testCases\\"   #样本
    pro_pre = "d_answer_testCases\\"    #全部
...
#with open('s_pro_dict.json',mode='w',encoding="utf8") as f:  #样本
with open('d_pro_dict.json',mode='w',encoding="utf8") as f:   #全部
    f.write(json_pro)
json_record=json.dumps(record_dict,ensure_ascii=False,indent=4, separators=(',', ': '))
#with open('s_record_dict.json','w') as f:     #样本
with open('d_record_dict.json', 'w') as f:    #全部
    f.write(json_record)